...
describe "Set slice operation" do
  [:dense, :yale, :list].each do |stype|
    context "for #{stype}" do
      ...
      Steps "set and unset a range of entries with single values" do

        if stype == :yale
          Then "verify correct arrangement of Yale IJA and A arrays" do
        ...
        Then "set and reset a single entry" do
          n = @m.clone
          old_val = @m[0,0]
          @m[0,0] = 100
        ...
        if stype == :yale
          ...
          Then "set a second row of entries" do
            n[2,0..2] = 0
            expect(n[2,0..2].to_flat_array).to eq([0,0,0])
            expect(n[1,0..2].to_flat_array).to eq([3,4,5])
          end

          Then "reset both rows of entries" do
            n[0,0..2] = [0,1,2]
            n[2,0..2] = [6,7,8]
            expect(n).to eq(@m)
          end
        ...
        Then "set upper left-hand 2x2 corner to 100" do
          m[0..1,0..1] = 100
        ...
        Then "set upper left-hand 2x2 corner to 0" do
          m[0..1,0..1] = 0
          if stype == :yale
        ...
        m = @m.clone
        Then "set lower left-hand 2x2 corner to 100" do
          m[1..2,0..1] = 100
        ...
        Then "set lower left-hand 2x2 corner to 0" do
          m[1..2,0..1] = 0
          expect(m[1..2,0..1]).to eq(slice_result_b)
        end

        m = @m.clone
        Then "set lower right-hand 2x2 corner to 100" do
          m[1..2,1..2] = 100
        ...
        Then "set lower right-hand 2x2 corner to 0" do
          m[1..2,1..2] = 0
          expect(m[1..2,1..2]).to eq(slice_result_b)
        end

        m = @m.clone
        Then "set upper right-hand 2x2 corner to 100" do
          m[0..1,1..2] = 100
        ...
        Then "set upper right-hand 2x2 corner to 0" do
          m[0..1,1..2] = 0
